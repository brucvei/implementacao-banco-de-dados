...
public class Main {
    ...
    public void testInsertion(Table table, long value) throws Exception {
        Record rec = table.addRecord(value, value + "added record");
        table.flushDB();
    ...
    public void testMultipleInsertions(Table table, int offset, int amount, boolean ordered, boolean display, int len) throws Exception {
        ...
        for (int i = 0; i < array.length; i++) {
            if (display) {
                System.out.println(i + "=>adding primary key =  " + array[i]);
            }
            String text = "Novo registros " + array[i];

            table.addRecord(array[i], Utils.pad(text, len));
    ...
    public void testMultipleSearches(Table table, int offset, int amount, boolean ordered, boolean display) throws Exception {
        ...
        for (int i = 0; i < array.length; i++) {
            ...
            if (rec != null) {
                ...
                System.out.println(rec.getContent().trim() + " in block " + rec.getBlockId());
            } else {
...
//            m.testMultipleSearches(table, 0, 1000, true, false);
            ...
            //m.testInsertion(table,23);   
            //m.testInsertion(table,24);
            //m.testInsertion(table,15);
